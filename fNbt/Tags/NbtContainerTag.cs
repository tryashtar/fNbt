...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
    public abstract class NbtContainerTag : NbtTag, IList<NbtTag>
    {
        ...
        {
            PerformChange(new DescriptionHolder("Add {0} to {1}", this, item),
                () => DoAdd(item),
                () => DoRemove(item)
            );
        }
        public void AddRange(IEnumerable<NbtTag> items)
        {
            ...
            PerformChange(new DescriptionHolder("Add {0} to {1}", this, items),
                () => DoAddRange(items),
        ...
        public void Clear()
        {
            ...
            PerformChange(new DescriptionHolder("Clear all tags from {0}", this),
                () => DoClear(),
                () => DoAddRange(list)
            );
        }
        ...
        {
            PerformChange(new DescriptionHolder("Insert {0} into {1} at index {2}", this, item, index),
                () => DoInsert(index, item),
                () => DoRemoveAt(index)
            );
        }
        public bool Remove(NbtTag item)
        {
            ...
            if (index == -1)
                return false;
            return PerformChange(new DescriptionHolder("Remove {0} from {1}", item, this),
                () => DoRemove(item),
                () => { DoInsert(index, item); }
            );
        }
        ...
        {
            var item = this[index];
            PerformChange(new DescriptionHolder("Remove {0} from {1}", item, this),
                () => DoRemoveAt(index),
                () => DoInsert(index, item)
            );
        }
